...
const Step2 = ({formHook}) => {
  const { register, getValues, setValue, watch } = formHook;
  ...
  return (
    <>
      ...
      <div className='lg:flex lg:flex-row lg:justify-between'>
        { 
          billingOptions.map(billingOption => {
            const styling = (watch('subscriptionType') === billingOption.type) ? 'border-[#473dff] bg-[#eef5ff]' : 'border-[#d6d9e6]';
            const price = (getValues('billingType') === 'Monthly') ? billingOption.monthly : billingOption.yearly;
            return (
              <label 
                className={`inline-block w-full h-min p-3 my-1.5 border rounded-lg ${styling} hover:cursor-pointer lg:w-[31%]`}
                onClick={() => { setValue('subscriptionType', billingOption.type); }}
      ...
      <MonthlyYearlyRadioButton formHook={formHook} />
    </>
  );
}
