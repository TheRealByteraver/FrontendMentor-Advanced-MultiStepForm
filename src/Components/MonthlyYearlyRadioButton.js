const MonthlyYearlyRadioButton = ({formHook}) => {
  const { register, watch, setValue } = formHook;
  return (
    <div className='text-center mt-4 text-sm font-bold bg-[#eef5ff] rounded-md p-3'>
      <div>
        <label 
          className={`w-1/4 mr-6 ${(watch('billingType') === 'Monthly' ? 'text-[#02295a]' : 'text-[#9699ab]')} hover:cursor-pointer`}
        ...
        <div className='inline -mt-[1px] pt-[2px] rounded-[12px] h-[26px] w-54px px-1 bg-[#02295a]'>
          ...
          <input 
            className='text-[#02295a] appearance-none rounded-full w-3 h-3 checked:bg-white hover:cursor-pointer' 
            onClick={() => { setValue('billingType', 'Yearly'); }}
            type='radio' id='Yearly' {...register('billingType')} value='Yearly' 
          />
        </div>
        <label 
          className={`w-1/4 ml-6 ${(watch('billingType') === 'Yearly' ? 'text-[#02295a]' : 'text-[#9699ab]')} hover:cursor-pointer`} 
